...
public class FootwearProductServiceImpl implements FootwearProductService {
    ...
    public FootwearProduct createProduct(FootwearProductRequest request) {
        ...
        Brand brand = brandRepository.findById(request.getBrandId())
                .orElseThrow(() ->
                        new IllegalArgumentException("Brand not found with id: " + request.getBrandId())
                );

        FootwearProduct product = new FootwearProduct();
        product.setBrand(brand);
        product.setSubBrand(request.getSubBrand());
        product.setArticle(request.getArticle());
        product.setCategory(request.getCategory());
        product.setKidsSubCategory(request.getKidsSubCategory());
